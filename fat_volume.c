...
static int fat_read_dos_2_0_bpb(struct fat_volume                       *vol,
...
static int fat_read_dos_3_31_bpb(struct fat_volume                      *vol,
                                 const struct fat_boot_sector_disk      *boot_sec)
{
...
static int fat_read_nonfat32_ebpb(struct fat_volume             *vol,
                                  const struct nonfat32_ebpb    *ebpb)
{
    ...
    vol->volume_id              = le32_to_cpu(ebpb->volume_id);
        memcpy( vol->volume_label,      ebpb->volume_label,
            sizeof(ebpb->volume_label));
        memcpy( vol->fs_type,           ebpb->fs_type,
            sizeof(ebpb->fs_type));
...
static int fat_read_fat32_ebpb(struct fat_volume        *vol,
                               const struct fat32_ebpb  *ebpb)
{
    ...
    vol->alt_boot_sector        = le16_to_cpu(ebpb->alt_boot_sector);
        ...
        DEBUG(  "root_dir_start_cluster = %u",
            vol->root_dir_start_cluster);
        DEBUG(  "fs_info_sector = %hu",                 vol->fs_info_sector);
        DEBUG(  "alt_boot_sector = %hu",                vol->alt_boot_sector);

    if (vol->fs_info_sector == 0xffff) vol->fs_info_sector = 0;
...
static int fat_read_boot_sector(struct fat_volume *vol, int fd)
{
    u8 buf[512];
    const struct fat_boot_sector_disk *boot_sec;
    int ret;
    ...
    if (full_pread(fd, buf, 512, 0) != 512) return -1;
    ...
    if (vol->num_data_clusters < 4085) vol->type = FAT_TYPE_FAT12;
    else if (vol->num_data_clusters < 65535) vol->type = FAT_TYPE_FAT16;
    else vol->type = FAT_TYPE_FAT32;
    return 0;
}

/* Map the first File Allocation Table into memory. */
static int fat_map_fat(struct fat_volume *vol, int fd, int mount_flags)
{
    long        page_size;
    void        *ptr;
    ...
    fat_size_bytes = (size_t)vol->sectors_per_fat <<
                     vol->sector_order;
    fat_aligned_size_bytes = fat_size_bytes +
                             (fat_offset - fat_aligned_offset);
    ...
    if (mount_flags & FAT_MOUNT_FLAG_READWRITE) prot |= PROT_WRITE;
 ...
 */
struct fat_volume* fat_mount(const char *volume, int mount_flags)
{
    ...
    if (mount_flags & FAT_MOUNT_FLAG_READWRITE) open_flags = O_RDWR;
    else open_flags = O_RDONLY;

    fd = open(volume, open_flags);

    if (fd < 0) goto out_free_vol;

    /* Read the FAT boot sector */
    ret = fat_read_boot_sector(vol, fd);

    if (ret) goto out_close_fd;
    ...
    vol->fd                     = fd;           /* File descriptor to use when
                                                   reading data */
    vol->mount_flags            = mount_flags;  /* Store the mount flags (not
                                                   yet used
                                                   anywhere though) */
    vol->max_allocated_files    = 100;          /* Arbitrary soft limit, to keep
                                                   memory
                                                   usage down. */
    ...
    if (vol->type ==
        FAT_TYPE_FAT32) vol->root.dentry.start_cluster =
            vol->root_dir_start_cluster;
    goto out;
out_close_fd:
    close(fd);
...
int fat_unmount(struct fat_volume *vol)
{
    int ret;
...
u32 fat_next_cluster(struct fat_volume *vol, u32 cur_cluster)
{
    ...
    if (!fat_is_valid_cluster_number(vol,
