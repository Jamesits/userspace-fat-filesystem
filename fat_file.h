...
struct fat_dir_entry {
    char name[8 + 1 + 3 + 1];   /* filename; with extension, if present;
                                   null-terminated */
    u8  attribs;
    u32 create_time;
    u32 create_date;
...
extern ssize_t
fat_file_pread(struct fat_file  *file,
               void             *buf,
               size_t           size,
               off_t            offset);
...
static inline bool fat_file_is_root(const struct fat_file *file)
{
    return file->parent == NULL;
}

static inline bool fat_file_is_directory(const struct fat_file *file)
{
