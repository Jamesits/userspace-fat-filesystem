...
static int _avl_cmp_fat_files(const struct avl_tree_node        *node1,
...
static int _avl_cmp_name_to_fat_file(const void                 *name,
...
static bool fat_char_legal_in_filename(char c)
{
...
static bool fat_base_name_valid(const u8 base_name[8])
{
    unsigned i;
    ...
    do {
        if (!fat_char_legal_in_filename(base_name[i])) return false;
 ...
 * */
static bool fat_extension_valid(const u8 extension[3])
{
    unsigned i;

    for (i = 0; i < 3 && extension[i] != '\0' && extension[i] != ' '; i++)
        if (!fat_char_legal_in_filename(extension[i])) return false;
...
static bool fat_is_end_of_directory(const struct fat_dir_entry_disk *disk_dentry)
{
...
static bool fat_ignore_dentry(const struct fat_dir_entry_disk *disk_dentry)
{
...
static bool fat_insert_child(struct fat_file *dir, struct fat_file *child)
{
...
static unsigned fat_name_len(const u8 *name, unsigned max_len)
{
    unsigned len = max_len;

    do {
        if ((name[len - 1] != ' ') && (name[len - 1] != '\0')) break;
    } while (--len);
    return len;
}
 ...
 * on-disk format. */
static void init_fat_dir_entry(struct fat_dir_entry             *dentry,
...
static void fat_vol_free_old_files(struct fat_volume *vol)
{
...
int fat_dir_read_children(struct fat_file *dir)
{
    u32 chunk_size;
    off_t cur_offset;
    u32 cur_cluster;
    ...
    for (;;) {
        ...
        if (full_pread(vol->fd, buf, chunk_size,
                       cur_offset) != chunk_size) goto err_destroy_tree;

        for (disk_dentry_ptr = (const struct fat_dir_entry_disk *)buf;
             disk_dentry_ptr <= end_ptr;
             disk_dentry_ptr++, dir_entries_processed++)
        {
            if (fat_is_end_of_directory(disk_dentry_ptr)) goto out_success;

            if (fat_ignore_dentry(disk_dentry_ptr)) continue;
            child = calloc(1, sizeof(*child));

            if (!child) goto err_destroy_tree;
        ...
        if (cur_cluster == FAT_CLUSTER_END_OF_CHAIN) {
            ...
            if (dir_entries_processed >= vol->max_root_entries) goto out_success;
            cur_offset += chunk_size;
        } else {
            cur_cluster = fat_next_cluster(vol, cur_cluster);

            if (cur_cluster == FAT_CLUSTER_END_OF_CHAIN) goto out_success;
            cur_offset = fat_data_cluster_offset(vol, cur_cluster);
        }
    }
out_success:
...
static struct fat_file* fat_dir_lookup(struct fat_file *dir, const char *name)
{
    struct avl_tree_node *node;

    if (!dir->children_read)
        if (fat_dir_read_children(dir)) return NULL;
...
static void fat_file_touch(struct fat_file *file)
{
    ...
    if ((file->subdir_num_times_opened == 0) && file->children_read) list_move(
            &file->lru_list,
            &file->volume->lru_file_list);
}
...
struct fat_file* fat_pathname_to_file(struct fat_volume *vol, const char *path)
{
    ...
    for (file = cur; file != NULL; file = file->parent) fat_file_touch(file);
    return cur;
}

static void fat_file_free(struct fat_file *file)
{
    if (fat_file_is_directory(file)) fat_destroy_file_tree(file);
    else fat_file_free_cluster_cache(file);
    free(file);
}

/* Free child `struct fat_files' (if any) of a `struct fat_file' */
void fat_destroy_file_tree(struct fat_file *root)
{
    struct fat_file *child;
...
int fat_file_alloc_cluster_cache(struct fat_file *file)
{
    u32 cluster_order;
    u32 num_clusters;
    u32 i;
    ...
    if (!file->file.cluster_cache) return -1;
    ...
    for (i = 1; i < num_clusters;
         i++) file->file.cluster_cache[i] = FAT_CLUSTER_END_OF_CHAIN;
    file->file.last_known_cluster_idx = 0;
    return 0;
}

/* Free the table of clusters for a FAT file */
void fat_file_free_cluster_cache(struct fat_file *file)
{
...
void fat_file_inc_num_times_opened(struct fat_file *file)
{
...
void fat_file_dec_num_times_opened(struct fat_file *file)
{
    file->num_times_opened--;
...
static time_t fat_time_to_unix_time(u16 date, u16 time)
{
 ...
 * stat'). */
int fat_file_to_stbuf(struct fat_file *file, struct stat *stbuf)
{
    memset(stbuf, 0, sizeof(*stbuf));
    stbuf->st_nlink = 1;

    if (file->dentry.attribs &
        FILE_ATTRIBUTE_DIRECTORY) stbuf->st_mode |= S_IFDIR;
    else stbuf->st_mode |= S_IFREG;

    if (file->dentry.attribs & FILE_ATTRIBUTE_READONLY) stbuf->st_mode |= 0555;
    else stbuf->st_mode |= 0777;
    stbuf->st_size      = file->dentry.file_size;
    stbuf->st_blocks    =
        (stbuf->st_size + (1 << file->volume->cluster_order) - 1)
...
void fat_file_init(struct fat_file *file, struct fat_volume *vol)
{
    file->volume = vol;
 ...
 * FAT. */
static int fat_file_preload_clusters(struct fat_file *file, u32 end_cluster_idx)
{
    u32 *cluster_cache;
    u32 next_cluster;
    ...
    while (file->file.last_known_cluster_idx < end_cluster_idx) {
        next_cluster = fat_next_cluster(file->volume,
                                        cluster_cache[file->file.
                                                      last_known_cluster_idx]);

        if (next_cluster == FAT_CLUSTER_END_OF_CHAIN) return -EIO;
...
static size_t do_fat_file_pread(struct fat_file *file,
                                void            *buf,
                                size_t          size,
                                off_t           offset,
                                ...
                                u32             end_cluster_idx)
{
    ...
    for (i = start_cluster_idx; i <= end_cluster_idx; i++) {
        ...
        if (bytes_read != cluster_needed_bytes) break;
...
ssize_t fat_file_pread(struct fat_file *file, void *buf, size_t size,
                       off_t offset)
{
    u32 start_cluster_idx;
    u32 end_cluster_idx;
